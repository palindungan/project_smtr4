...
--
-- Host: 127.0.0.1
-- Waktu pembuatan: 07 Jul 2019 pada 13.32
...
-- Fungsi
--
CREATE DEFINER=`root`@`localhost` FUNCTION `GetCountBonus` () RETURNS INT(10) BEGIN
    DECLARE countx INT(10);
 
    SELECT
        COUNT(tb.id_bonus) into countx
    FROM 
    	tbl_bonus tb;
...
CREATE DEFINER=`root`@`localhost` FUNCTION `GetCountKategori` () RETURNS INT(10) BEGIN
    DECLARE countx INT(10);
 
    SELECT
        COUNT(tk.id_kat) into countx
    FROM 
    	tbl_kategori tk;
...
CREATE DEFINER=`root`@`localhost` FUNCTION `GetCountKeterangan` () RETURNS INT(10) BEGIN
    DECLARE countx INT(10);
 
    SELECT
        COUNT(tkp.id_keterangan_paket) into countx
    FROM 
    	tbl_keterangan_paket tkp;
 
 RETURN (countx);
END$$

CREATE DEFINER=`root`@`localhost` FUNCTION `GetCountMenu` () RETURNS INT(10) BEGIN
    DECLARE countx INT(10);
 
    SELECT
        COUNT(tm.id_menu) into countx
    FROM 
    	tbl_menu tm;
...
CREATE DEFINER=`root`@`localhost` FUNCTION `GetCountPaket` () RETURNS INT(10) BEGIN
    DECLARE countx INT(10);
 
    SELECT
        COUNT(tp.id_paket) into countx
    FROM 
    	tbl_paket tp;
...
CREATE DEFINER=`root`@`localhost` FUNCTION `GetCountUserAndroid` () RETURNS INT(10) BEGIN
    DECLARE countx INT(10);
 
    SELECT
        COUNT(tc.id_customer) into countx
    FROM 
    	tbl_customer tc;
...
CREATE DEFINER=`root`@`localhost` FUNCTION `GetCountUserWeb` () RETURNS INT(10) BEGIN
    DECLARE countx INT(10);
 
    SELECT
        COUNT(u.id_user) into countx
    FROM 
    	user u;
...
('PR-0000003', '1705190002', 'PK-004', 3, 3000, 1500, 3000, 'PR-0000003.jpeg', 'sdwdew', '2019-06-24', '2019-08-20', '2019-08-27', 'belum_lunas'),
